...
$("#adminForm").on("submit", function (event) {
  ...
  if (btnAction == "Insert") {
    form_data.append("action", "register_admin");
  } else {
    form_data.append("action", "update_admin");
  }
...
function DisplayMessage(type, message) {
  ...
  if (type == "success") {
    ...
    //in model qarsoomo
    setTimeout(function () {
      $("#adminModal").modal("hide"); //model qarsomo
      $("#adminForm")[0].reset(); // xogta form tiraa
...
function loadData() {
  $("#adminTable tbody").html(""); //marka  loadData la waco markasta xogta tableka nadiifiya si xogta cusub u soo baxdo
...
$("#adminTable").on("click", "a.update-info", function () {
  let id = $(this).attr("update_id");
  fetchUserInfo(id);
});

$("#adminTable").on("click", "a.delete-info", function () {
